...
"""
    ...
    async def evaluate_program(self, program: Union[Program, StrategyProgram], task: TaskDefinition) -> Union[Program, StrategyProgram]:
        ...
        # Run backtest if strategy evaluation
        if task.task_type == "strategy":
            try:
                backtest_results = self.backtester.run_backtest(
                    strategy_func=program.code,
                    symbol=task.backtest_symbol,
                    start_date=task.backtest_start_date,
                    end_date=task.backtest_end_date,
                    initial_capital=task.initial_capital
                )
                program.fitness_scores.update(backtest_results)
                
                # Additional strategy-specific metrics
                if isinstance(program, StrategyProgram):
                    # Get market regime from MCP server
                    try:
                        regime_result = await self._get_market_regime()
                        program.market_regime = regime_result.get("regime")
                        program.regime_confidence = regime_result.get("confidence", 0.0)
                        
                        # Store backtest results with regime context
                        program.backtest_results = {
                            "start_date": task.backtest_start_date,
                            "end_date": task.backtest_end_date,
                            "metrics": backtest_results,
                            "regime": program.market_regime,
                            "confidence": program.regime_confidence
                        }
                        
                        # Add to version history
                        program.version_history.append({
                            "timestamp": time.time(),
                        ...
                        program.errors.append(f"Market regime analysis failed: {str(e)}")
                
                logger.info(f"Backtest results for program {program.id}: {backtest_results}")
            except Exception as e:
                program.errors.append(f"Backtest failed: {str(e)}")
                logger.error(f"Backtest error for program {program.id}: {e}")
    ...
    async def _get_market_regime(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if regime_memories and regime_memories["results"]:
                # Use the most recent memory
                latest = regime_memories["results"][0]
                return {
                    "regime": latest.get("memory", {}).get("regime", "unknown"),
                    "confidence": latest.get("memory", {}).get("confidence", 0.0),
                    "source": "mem0",
                    "timestamp": latest.get("timestamp")
                }
            
            # Fallback to default if no memories found
            return {"regime": "bullish_trending", "confidence": 0.85}
        except Exception as e:
            logger.error(f"Failed to get market regime from mem0: {e}")
